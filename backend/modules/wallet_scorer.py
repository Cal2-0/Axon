...
Use ALL of this evidence to form a comprehensive forensic assessment. Weigh behavioral patterns, OSINT reputation, and financial flow data together."""
    ...
    response_data = {
        ...
        "identity": {
            ...
            "nativeBalance": eth_balance_str,
            "nativeSymbol": "ETH",
            "totalReceived": f"{total_received_eth:.4f} ETH" if total_received_eth > 0 else "0",
            "totalSent": f"{total_sent_eth:.4f} ETH" if total_sent_eth > 0 else "0",
            "nativeTotalReceived": f"{total_received_eth:.4f}" if total_received_eth > 0 else "0",
            "nativeTotalSent": f"{total_sent_eth:.4f}" if total_sent_eth > 0 else "0",
            "txCount": tx_count,
